...
#include "../Library/Object/Setters/Setters.h"
...
/*
 * Se deberá iterar todos los elementos de la lista y pasárselos a la funcion.La función “fn”
 * devolverá
...
LinkedList* ll_map(LinkedList* this, float (*pFunc)(void*)){
	...
	if(this!=NULL && *pFunc!=NULL){
		...
		for (int i = 0; i < lenght; i++) {
			...
			power = pFunc(pElement); //pongo su retorno en la variable
			if(power!=-1){ // si da -1, fallo y no copio nada
				Object_setCosmosLevel(pElement, power); //le pongo el nuevo power al elemento
				ll_add(mappedList, pElement); //lo agrego a la lista
			}
		}
	}
