...
int Object_getHigherID(Object *this, int *id) {
    int sucess = -1;
...
int saveAsText_maxID(FILE *pFile, LinkedList *this,
		int *maxID) {
	...
	if (pFile != NULL && this != NULL) {//si el archivo y el array no son null..
        ...
        for(int i=0; i<len_LL; i++){	// recorro el array.
			anotherObject = ll_get(this, i);// copio la estructura del empleado del indice.
			Object_getHigherID(anotherObject, &ID);//obtengo toda la info del empleado.
...
int controller_saveAsTextMaxID(FILE *pFile, char *path,
	...
	if (pFile != NULL
			&& saveAsText_maxID(pFile, this, &maxID)) { //si el archivo no es null y pude escribir, retorno 1
        sucess = 1;
